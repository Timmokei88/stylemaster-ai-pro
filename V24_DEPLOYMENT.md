# MixoLabs V24 deployment

This update adds a dedicated narrated tutorial step for the scalable payment controls. It opens Plans & Credits, highlights the custom pack builder, and explains the slider, exact pound-and-pence entry, live credit estimate, one-time packs, monthly plans and the final Stripe confirmation boundary.

It also removes the unsupported 512 image-size option for the configured Gemini Lite model. The minimum supported output is now 1K at one credit per image; failed generation requests continue to restore credits automatically.
