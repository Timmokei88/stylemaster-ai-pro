...
const checks=[
 ...
 [server.includes('currentProduct.active===false||currentProduct.deleted===true'),'inactive or deleted Stripe products are detected'],
 [server.includes('stripe.products.update(productId,{active:true})'),'an archived Stripe product is reactivated before the plan change'],
 [server.includes('stripe.products.create({name:"MixoLabs Custom Monthly"'),'a new active product is available if reactivation fails'],
 [server.includes('custom-price:${subscription.id}:${productId}:${amountPence}'),'replacement prices use product-aware idempotency'],
